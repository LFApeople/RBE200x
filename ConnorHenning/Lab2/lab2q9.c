...
int main() {
	...
	// Sets and initializes the pin as an output.
	const int IO_PIN = 22;
	gpio_init(IO_PIN);
	...
	while(true) {
		...
		for (i = 1; i <= 5; i++) {
		// A basic function to set pin 32 (GPIO22) to an on state
		gpio_put (IO_PIN, 1) ;
		// The chip is set to wait for half a second (500 ms) of no activity.
		sleep_ms(500);
		// A function to set pin 32 (GPIO22) to an off state
		gpio_put (IO_PIN,0);
		// The chip is set to wait for half a second (500 ms) of no activity.
		sleep_ms(500);
	}
	...
	for (i = 1; i <= 5; i++) {
		// A function to set pin 32 (GPIO22) to an on state
		gpio_put (IO_PIN, 1) ;
		// The chip is set to wait for half a second (500 ms) of no activity.
		sleep_ms(100);
		// A function to set pin 32 (GPIO22) to an off state
